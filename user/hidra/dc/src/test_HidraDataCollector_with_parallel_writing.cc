...
public:
  ...
  ~HidraDataCollector() override { StopWriter(); }
...
private:
  ...
  uint64_t m_incomplete_flush_period_us = 1000;
  uint64_t m_last_incomplete_flush_ns = 0;
  ...
  std::condition_variable m_writer_space_cv;
  std::deque<eudaq::EventSP> m_write_queue;
  bool m_writer_stop = false;
  bool m_writer_active = false;
  bool m_writer_failed = false;
  std::string m_writer_error;
  size_t m_writer_max_queue_events = 4096;
  size_t m_writer_flush_every_events = 32;
  uint64_t m_writer_flush_interval_ms = 50;
  uint64_t m_writer_written_events = 0;
  ...
  void FlushOldIncompleteEvents(bool force = false) {
    const uint64_t now_ns = hidra::utils::getTimens();
    if (!force && m_last_incomplete_flush_ns != 0 &&
        now_ns - m_last_incomplete_flush_ns < m_incomplete_flush_period_us * 1000) {
      return;
    }
    m_last_incomplete_flush_ns = now_ns;
    ...
    for (auto it = m_pending_events.begin(); it != m_pending_events.end();) {
      ...
      if (!m_is_replay_mode) {
        age_ns = now_ns - it->second.first_seen_ns;
      } else {
      ...
      if (EnqueueEventForWriting(std::move(mergedEvt))) {
        ++m_event_count;
      }
      it = m_pending_events.erase(it);
    }
  ...
  void StartWriter() {
...

    {
      ...
      m_writer_error.clear();
      m_writer_written_events = 0;
    }

    m_writer_thread = std::thread(&HidraDataCollector::WriterLoop, this);
  }

  void StopWriter() {
    {
      std::lock_guard<std::mutex> lock(m_writer_mutex);
      if (!m_writer_active && !m_writer_thread.joinable()) {
        return;
      }
      m_writer_stop = true;
    }
    m_writer_cv.notify_all();
    m_writer_space_cv.notify_all();
  ...
  bool EnqueueEventForWriting(eudaq::EventSP event) {
    if (!event) {
      return false;
    }
    ...
    if (!m_writer_active) {
      HIDRA_ERROR("Writer thread is not active");
      return false;
    }

    if (m_writer_failed) {
      HIDRA_ERROR("Writer thread is in failed state: {}", m_writer_error);
      return false;
    }

    m_writer_space_cv.wait(lock, [this] {
      return m_writer_stop || m_writer_failed || m_writer_max_queue_events == 0 ||
             m_write_queue.size() < m_writer_max_queue_events;
    });

    if (m_writer_stop || m_writer_failed) {
      if (m_writer_failed) {
        HIDRA_ERROR("Writer thread is in failed state: {}", m_writer_error);
      }
      return false;
    ...
    m_write_queue.push_back(std::move(event));
    lock.unlock();
  ...
  void WriterLoop() {
    ...
    while (true) {
      ...
      {
        std::unique_lock<std::mutex> lock(m_writer_mutex);
        m_writer_cv.wait_for(lock, std::chrono::milliseconds(m_writer_flush_interval_ms), [this] {
          return m_writer_stop || !m_write_queue.empty();
        });

        should_stop = m_writer_stop && m_write_queue.empty();
        if (should_stop) {
          break;
        }

        if (!m_write_queue.empty()) {
          event = std::move(m_write_queue.front());
          m_write_queue.pop_front();
          m_writer_space_cv.notify_one();
        }
      }

      try {
        if (event) {
          hidra::EventSerializer::WriteToStream(*event, m_hidraOutput);
          ++events_since_flush;
          ++m_writer_written_events;
        }

        const auto now = std::chrono::steady_clock::now();
        const bool flush_by_batch =
            m_writer_flush_every_events > 0 && events_since_flush >= m_writer_flush_every_events;
        const bool flush_by_time =
            std::chrono::duration_cast<std::chrono::milliseconds>(now - last_flush).count() >=
            static_cast<int64_t>(m_writer_flush_interval_ms);
        const bool flush_when_idle = !event;

        if ((flush_by_batch || flush_by_time || flush_when_idle) && events_since_flush > 0) {
          m_hidraOutput.flush();
        ...
        m_writer_error = ex.what();
        HIDRA_ERROR("Writer thread failed: {}", m_writer_error);
        m_writer_space_cv.notify_all();
        return;
      }
    }
    ...
    if (events_since_flush > 0) {
      try {
        ...
        HIDRA_ERROR("Writer thread final flush failed: {}", ex.what());
      }
    }
  }
  ...
  void DoConfigure() override {
    ...
    m_incomplete_flush_period_us = conf->Get("INCOMPLETE_FLUSH_PERIOD_US", 1000);
    m_tstamp_window_ns = conf->Get("TIMESTAMP_WINDOW_NS", 50000);
    const int writer_max_queue_events = conf->Get("WRITER_MAX_QUEUE_EVENTS", 4096);
    const int writer_flush_every_events = conf->Get("WRITER_FLUSH_EVERY_EVENTS", 32);
    const int writer_flush_interval_ms = conf->Get("WRITER_FLUSH_INTERVAL_MS", 50);
    m_writer_max_queue_events = writer_max_queue_events < 0 ? 4096 : static_cast<size_t>(writer_max_queue_events);
    m_writer_flush_every_events =
        writer_flush_every_events < 0 ? 32 : static_cast<size_t>(writer_flush_every_events);
    m_writer_flush_interval_ms =
        writer_flush_interval_ms < 0 ? 50 : static_cast<uint64_t>(writer_flush_interval_ms);
    if (m_writer_flush_interval_ms == 0) {
      HIDRA_WARN("WRITER_FLUSH_INTERVAL_MS=0 would busy-wait. Using 1 ms instead.");
      m_writer_flush_interval_ms = 1;
    }
  ...
  void DoStartRun() override {
    ...
    if (!m_hidraOutput) {
      HIDRA_ERROR("Cannot open Hidra output file temp_data.dat");
      return;
    }
    StartWriter();
    m_running = true;
